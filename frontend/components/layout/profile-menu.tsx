        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="space-y-3">
            ...
            <Input
              ...
              className="bg-black border-zinc-800 h-11 rounded-lg focus-visible:ring-zinc-700"
            />
          </div>
          <div className="space-y-3">
    ...
    <Dialog open={open} onOpenChange={(next) => { onOpenChange(next); if (!next) reset(); }}>
      <DialogContent className="bg-zinc-950 border-zinc-900 text-white sm:max-w-md rounded-xl p-6">
        ...
        {!code ? (
              ...
              <div className="grid grid-cols-3 gap-2">
                {ROLES.map((r) => (
                  <button
                    ...
                    type="button"
                    onClick={() => setRole(r)}
                    className={`h-10 rounded-md text-xs font-semibold uppercase tracking-wider border transition-colors ${role === r ? 'bg-white text-black border-white' : 'bg-black border-zinc-800 text-zinc-400 hover:text-white hover:border-zinc-600'}`}
                  >
                    {r}
                  </button>
                ))}
          ...
          <div className="space-y-6">
            <div className="space-y-3">
              <label className="text-xs font-medium text-zinc-400 uppercase tracking-wider">{role} Invite Code</label>
              <div className="flex gap-2">
                <Input
                  readOnly
                  value={code}
                  onFocus={(e) => e.target.select()}
                  className="bg-black border-zinc-800 h-11 rounded-lg font-mono text-sm"
                />
                <Button
                  type="button"
                  variant="secondary"
                  ...
                  className="bg-zinc-900 hover:bg-zinc-800 text-white rounded-md h-11 w-11 p-0 border border-zinc-800 shrink-0"
                >
            ...
            <Button
              ...
              variant="outline"
              className="w-full h-12 rounded-md border-zinc-800 bg-zinc-900 hover:bg-zinc-800 text-white text-sm font-bold tracking-widest uppercase"
            >
              Close
            </Button>
          </div>
